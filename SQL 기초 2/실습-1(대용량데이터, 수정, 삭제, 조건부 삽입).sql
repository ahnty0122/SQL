...
-- update문으로 다시 수정을 해서 아래와 같이 사용을 하면 안전하다.
update testtbl4
   set lname = '없음'
 where fname = 'Kyoichi';
...
-- 하지만, 현업에서는 drop,truncate구문은 잘 사용하지 않는다.
drop table bigtbl1;
delete from bigtbl2;
...
-- 테이블을 위와 같이 만들면, 제약조건은 따로 복사가 안된다고 앞에서 강의한 적이 있다.
-- 하여, alter구문을 이용해서 직접 제약조건을 설정을 해보도록 하자.
desc membertbl;
desc usertbl;
-- alter라는 DDL구문은 테이블을 수정할 때 사용한다.
-- 아래는 membertbl테이블에 PK를 추가하는 구문이다.
...
-- 데이터를 삽입하기 위해 아래와 같이 코드를 작성을 했다.
-- 근데 삽입이 안된다.그 이유는 PK중복 때문에 안된다.
...
-- insert ignore into구문은 PK가 중복되는 것을 무시하고 아래 2개의 삽입은 
-- 이루어지게 했다.
...
-- 하지만,OCP라던지 SQLD는 자격증 시험에는 한 번씩 나오므로 이론적으로 알아둘 필요는 있다.
insert into membertbl values ('BBK', '바비코', 'LA')
