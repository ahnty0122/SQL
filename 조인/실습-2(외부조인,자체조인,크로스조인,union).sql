...
-- 그런데 위의 두 개의 결과값을 동시에 보고싶을때는 어떻게 할까?
-- 두 쿼리의 결과값을 연결해주는 키워드가 필요한다.
-- 그게 바로 union이다.union은 합집합이다. 중복은 제거를 한다.   
select S.stdname, S.addr, C.clubname, C.roomno
  from stdtbl S
  left outer join stdclubtbl SC
  on S.stdname = SC.stdname
...
-- 카티션곱(Cartesian Product)이라고도 부른다.통상적으로 대용량데이터를 생성할때 사용한다.
